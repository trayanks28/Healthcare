...
// Fetch invoice data (FINAL FIXED VERSION)
$invoiceQuery = "
    SELECT 
        ...
        COALESCE(income.amount_paid, 0) AS amount_paid
    FROM appointment
    LEFT JOIN doctor ON appointment.doctor_id = doctor.id
    LEFT JOIN income ON income.appointment_id = appointment.id
...
if ($invoiceResult && $invoiceResult->num_rows > 0) {
    while ($row = $invoiceResult->fetch_assoc()) {
        ...
        // Handle missing income properly
        if ($row['amount_paid'] == 0) {
            $row['amount_paid'] = "Not Generated";
        }
